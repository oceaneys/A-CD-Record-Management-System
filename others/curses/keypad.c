...
int main()
{
	...
	noecho();
	clear();
	mvprintw(5, 5, "Key pad demonstraton. Press 'q' to quit");
	move(7, 5);
	refresh();
	...
	while(key != ERR && key != 'q'){
		...
		if((key >= 'A' && key <= 'Z') || (key >= 'a' && key <= 'z')){
			...
			switch(key){
				...
				case KEY_LEFT: printw("%s","LEFT key");printw("%o",key);break;
				case KEY_UP: printw("%s","UP key");break;
				case KEY_DOWN: printw("%s","DOWN key");break;
				case KEY_ENTER:printw("%s","ENTER key");break;
				default: printw("Unmatched - %o", key);break;
			}
		}
